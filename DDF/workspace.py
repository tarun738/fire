...
def load_model_parameters(experiment_directory, checkpoint, decoder, color, deform, ref):
    ...
    if color:
        filename = os.path.join(experiment_directory, model_params_subdir, checkpoint + "_dirs.pth")
    elif deform:
        filename = os.path.join(experiment_directory, model_params_subdir, checkpoint + "_points.pth")
    elif ref:
...
def load_latent_vectors_geometry(experiment_directory, checkpoint):
    ...
    latent_codes = load_lat_vecs(filename)

    filename2 = os.path.join(
        experiment_directory, latent_codes_subdir, checkpoint + "_ddf.pth"
    )

    if os.path.isfile(filename2):
        latent_codes_ddf = load_lat_vecs(filename2)
        return latent_codes, latent_codes_ddf
